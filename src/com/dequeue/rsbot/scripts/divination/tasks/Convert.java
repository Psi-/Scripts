 ...
 * User: Dequeue
 */
public class Convert extends Task<DQDivination> {
    ...
    public Convert(DQDivination script, MethodContext ctx) {
        super(script, ctx);
    }
    ...
    public boolean activate() {
        return (ctx.widgets.get(CONVERT_WIDGET_ID, MEMORY_TO_EXP_COMPONENT_ID).isValid() &&
                ctx.widgets.get(CONVERT_WIDGET_ID, MEMORY_TO_EXP_COMPONENT_ID).isVisible()) ||
    ...
    public void execute() {
        ...
        if (ctx.widgets.get(CONVERT_WIDGET_ID, componentId).isValid() &&
                ctx.widgets.get(CONVERT_WIDGET_ID, componentId).isValid()) {
                ...
                if (!Condition.wait(new Callable<Boolean>() {
                    ...
                    public Boolean call() throws Exception {
                        return ctx.widgets.get(CONVERT_WIDGET_ID, componentId).isValid() &&
                                ctx.widgets.get(CONVERT_WIDGET_ID, componentId).isVisible();
                    }
                }, 500, 8)) {
                    return;
                }
